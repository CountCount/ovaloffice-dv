...
print '<ul id="sub-rb" class="subtabs"><li><a href="#rb_ruins">'.t('RB_RUINS').'</a></li><li><a href="#rb_travel">'.t('RB_TRAVEL').'</a></li></ul>';
	?>
<div id="rb_travel" class="subtabcontent hideme">
...
<div id="rb_ruins" class="subtabcontent">
<h3><?php print t('RB_RUINS'); ?></h3>
<h4><?php print t('RB_RUINS_SUB'); ?></h4>
<?php
	$q = ' SELECT * FROM dvoo_zones_buildings WHERE tid = '.$data['town']['id'].' ';
	$r = $db->query($q);
	$ruins = array();
	if ( is_array($r) && count($r) > 0 ) {
		foreach ( $r AS $b ) {
			$ruins[get_dir_name($b['x'],$b['y'],$data['town']['x'],$data['town']['y'])][] = $b;
		}
		$zebra = 0;
		foreach ( $ruins AS $dir => $rdir ) {
			$zebra++;
			print '<div class="ruindir '.($zebra % 2 == 0 ? 'even' : 'odd').'"><h4>'.$dir.'</h4><ul class="ruins clearfix">';
			foreach ( $rdir AS $b ) {
				print '<li>'.get_ap($b['x'],$b['y'],$data['town']['x'],$data['town']['y']).'<img src="'.t('GAMESERVER_SMILEY').'h_ap.gif" title="'.t('AP').'" />&nbsp;<strong>'.$b['name'].'</strong>&nbsp;<span class="coords">['.($b['x'] - $data['town']['x']).'|'.($data['town']['y'] - $b['y']).']</span> ('.get_km($b['x'],$b['y'],$data['town']['x'],$data['town']['y']).'km)<br/>'.t('RBR_STATUS').' '.($b['dig'] > 0 ? $b['dig'].'x <img src="'.t('GAMESERVER_SMILEY').'h_dig.gif" title="Haufen" />' : ($b['depleted'] == 1 ? '<span class="empty">'.t('R_BUILDING_DEPLETED').'</span>' : '<span class="not-empty">'.t('R_BUILDING_REGENERATED').'</span>'));
				$items = unserialize($b['items']);
				if ( is_array($items) && count($items) > 0 ) {
					print '<div class="ruin-items clearfix">Bisherige Funde:<br/>';
					foreach ( $items AS $i ) {
						$q = ' SELECT iname AS name, iimg AS img FROM dvoo_items WHERE iid = '.$i['id'].' ';
						$r = $db->query($q);
						$imd = $r[0];
						print '<div title="'.$imd['name'].'" class="ruin-item" style="background:transparent url('.t('GAMESERVER_ITEM').$imd['img'].'.gif) 0px 1px no-repeat;">x'.$i['count'].'</div>';
					}
					print '</div>';
				}
				print ($b['content'] != '' ? '<br/><em>'.nl2br($b['content']).'</em>' : '').'</li>';
			}
			print '</ul></div>';
		}
		print '</div>';
...
?>
	...
	<script type="text/javascript">				
					var rb_cur = "#rb_ruins";
					$("ul#sub-rb li a").click(function (e) { 
						e.preventDefault();
						var newRB = $(this).attr("href");
						$(rb_cur).fadeOut();
						$(newRB).fadeIn("slow");
						rb_cur = newRB;
					});
...
$piwikTracker->setURL('http://dv.sindevel.com/oo/rb.main.ajax.php');
...
$piwikTracker->doTrackPageView('OO Travel');