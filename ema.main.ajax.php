...
if ( $data = unserialize($session[0]['xml']) ) {
	...
	print '<ul id="sub-ema" class="subtabs">
		...
		<li><a href="#ema_newregister">'.t('EMA_REGISTER_NEW').'</a></li>
		<li><a href="#ema_register">'.t('EMA_REGISTER').'</a></li>
	</ul>';

	print '<div id="ema_form" class="subtabcontent">';
...
<div id="ema_newregister" class="subtabcontent hideme">
	...
	<?php
		...
		foreach ( $citlist AS $cp ) {
			...
			print '<tr id="cp'.$cp['id'].'" class="citizen-row '.($z % 2 == 0 ? 'even' : 'odd').'">
				<td class="cr-avatar"><img class="small-avatar" src="'.$cp['avatar'].'"></td>
				<th class="cr-name"><div class="stats-wrapper"><div class="stats">'.minispy($cp['id']).'</div></div><img class="job" src="'.t('GAMESERVER_ITEM').$jobs[$cp['job']]['img'].'.gif" /> '.$cp['name'].($cp['ban'] == 1 ? ' <img src="'.t('GAMESERVER_SMILEY').'h_ban.gif" />' : '').'</th>
				...
				<td class="cr-lastlogin">'.($ll > 0 ? (time() - $ll < 60 ? 'vor wenigen Momenten' : (time() - $ll < 3660 ? 'vor '.floor((time() - $ll)/60).' Minuten' : (time() - $ll < 90000 ? 'vor '.floor((time() - $ll)/3600).' Stunden' : ('vor '.floor((time() - $ll)/86400).' Tagen')))) : 'noch nie').'</td>
			</tr>';
		}
...
function minispy($u) {
	...
	/* ### OUTPUT ### */
	$out = '<h2>'.$citizen['name'].'</h2>';
	/*$out .= '<div class="rewards-wrapper">';
	foreach ( $rare AS $r ) {
