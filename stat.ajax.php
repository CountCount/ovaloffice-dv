...
print '<ul class="subtabs" id="sub-stat">
	...
	<li><a href="#stat_records">'.t('STAT_RECORDS_TITLE').'</a></li>
	<li><a href="#stat_scouts">'.t('STAT_SCOUTS_TITLE').'</a></li>
...
foreach ($rewards AS $r) {
	if ( $r['img'] == 'easteregg' ) {
		print '<div class="reward'.($r['rare'] == 1 ? ' rare' : '').'"><img src="/ovaloffice/css/img/easteregg.png" title="'.$r['name'].'" onclick="rlist(\''.$r['name'].'\','.$data['user']['id'].');" /></div>';
	}
	elseif ( $r['img'] == 'ghostofsanta' ) {
		print '<div class="reward'.($r['rare'] == 1 ? ' rare' : '').'"><img src="/ovaloffice/css/img/ghostofsanta.gif" title="'.$r['name'].'" onclick="rlist(\''.$r['name'].'\','.$data['user']['id'].');" /></div>';
	}
	else {
...
foreach ($titles AS $t) {
	...
	$i++;
	if ( $t['img'] == 'easteregg' ) {
		print '<div class="rtitle'.($t['rare'] == 1 ? ' rare' : '').'" onclick="tlist(\''.$t['name'].'\','.$data['user']['id'].');" ><img src="/ovaloffice/css/img/easteregg.png" title="'.$t['name'].'" />&nbsp;'.$t['name'].'</div>';
	}
	elseif ( $t['img'] == 'ghostofsanta' ) {
		print '<div class="rtitle'.($t['rare'] == 1 ? ' rare' : '').'" onclick="tlist(\''.$t['name'].'\','.$data['user']['id'].');" ><img src="/ovaloffice/css/img/ghostofsanta.gif" title="'.$t['name'].'" />&nbsp;'.$t['name'].'</div>';
	}
	else {
...
// Scouts
print '<div id="stat_scouts" class="subtabcontent hideme">';
print '<h4>'.t('STAT_SCOUTS_TITLE').'</h4>';
print '<div class="scouts">';

$q = ' SELECT ze, zc, gc FROM dvoo_stat_camouflage ';
$r = $db->query($q);
$camo = array();
$mc = 0;
foreach ( $r AS $s ) {
	if ( isset($camo[$s['zc']][$s['ze']]) ) {
		$camo[$s['zc']][$s['ze']]['n']++;
		if ( $s['gc'] == 1 ) {
			$camo[$s['zc']][$s['ze']]['f']++;
		}
		...
		$camo[$s['zc']][$s['ze']] = array('n' => 1, 'f' => ($s['gc'] == 1 ? 1 : 0));
	}
	$mc = ($s['zc'] > $mc ? $s['zc'] : $mc);
}
krsort($camo);
...
print '<tr><th>'.t('ZOMBIES').'</th><th>'.t('EXPLORATION_LEVEL').' 0</th><th>'.t('EXPLORATION_LEVEL').' 1</th><th>'.t('EXPLORATION_LEVEL').' 2</th><th>'.t('EXPLORATION_LEVEL').' 3</th></tr>';
for ( $xc = $mc; $xc > 0; $xc-- ) {
	$zebra = ( $xc % 2 == 0 ? 'even' : 'odd' );
	...
	if ( isset($camo[$xc]) ) {
		...
		for ( $xe = 0; $xe < 4; $xe++ ) {
			if ( isset($camc[$xe]) ) {
				$d = $camc[$xe];
				print '<td>'.$d['f'].'/'.$d['n'].' ('.(round($d['f']/$d['n']*100)).'%)</td>';
			}
			else {
				print '<td></td>';
			}
...
foreach ( $seasons AS $sno => $sdates ) {
	print '<li id="otl-s'.$sno.'"><a href=".ot-s'.$sno.'">'.t('SEASON').' '.$sno.'</a></li>';
}
print '</ul>';

$j = array();
foreach ($seasons AS $s => $data) {
	$j[$s] = 1;
}
$oldtown_seasons = array();
foreach ( $todata AS $t ) {
	$town_season_number = getTownSeasonNumber($t['stamp'], $t['day'], $seasons);
	$town_data = '<tr class="'.($j[$town_season_number] % 2 == 0 ? 'even' : 'odd').(in_array($t['id'],$own_towns) ? ' own' : ' not-own').'"><td class="tr">'.$j[$town_season_number].'.</td><td>'.($t['hard'] == 1 ? '<img src="http://data.dieverdammten.de/gfx/icons/r_suhard.gif" />' : '<img src="http://data.dieverdammten.de/gfx/icons/r_surlst.gif" />').'&nbsp;'.$t['name'].'</td><td>'.($t['id'] >= 4560 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$t['id'].','.$t['day'].');" />' : '').'</td><td>'.($t['id'] > 6325 ? '<a href="http://dieverdammten.net/fatamorgana/spy/town/'.$t['id'].'" target="_blank"><img src="http://www.dieverdammten.de/file/29.dat" /></a>' : '').'</td><td class="tr">'.getPoints($t['id'], $t['day']).'</td><td class="tr">'.$t['day'].'</td><td class="tc">'.($t['chaos'] ? '<img src="css/img/check.png" title="'.t('STAT_CHAOS_X').'">' : '').'</td><td class="tc">'.($t['devast'] ? '<img src="css/img/check.png" title="'.t('STAT_DEVAST_X').'">' : '').'</td></tr>'; // '.getPoints($t['id'], $t['day']).'
	$oldtown_seasons[$town_season_number] .= $town_data;
	$j[$town_season_number]++;
}

foreach ($oldtown_seasons AS $sno => $data) {
	print '<table id="stat-towns-table-old" class="stat-towns-table ot-s'.$sno.($sno < max(array_keys($seasons)) ? ' hideme' : '').'" border="0">';
	print '<tr><th></th><th>'.t('STAT_CITY_NAME').'</th><th style="font-size:.75em;text-align:center;">Spy OO</th><th style="font-size:.75em;text-align:center;">Spy FM</th><th>'.t('POINTS').'</th><th>'.t('STAT_LAST_DAY').'</th><th>'.t('STAT_CHAOS').'</th><th>'.t('STAT_DEVAST').'</th></tr>';
	print $data;
	print '</table>';
}
print '</div>';
...
// Records
print '<div id="stat_records" class="subtabcontent hideme">';
print '<h4>'.t('STAT_RECORDS_TITLE').'</h4>
<p>'.t('STAT_RECORDS_INFO').'</p><div>';

$j = array();
foreach ($seasons AS $s => $sdates) {
	$j[$s] = 1;
	// highest zombie attack
	$r1_sql = $db->query(' 
			SELECT x.z, t.name, t.id, x.day 
			FROM dvoo_stat_zombies x 
			INNER JOIN dvoo_towns t ON t.id = x.tid  
			WHERE x.day > 0 '.
			(!is_null($sdates['start']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) > '.$sdates['start'] : '').
			(!is_null($sdates['end']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) < '.$sdates['end'] : '').
			' ORDER BY x.z DESC LIMIT 1');
	$r1 = $r1_sql[0];
	// highest defense
	$r2_sql = $db->query(' 
			SELECT x.v, t.name, t.id, x.day 
			FROM dvoo_stat_zombies x 
			INNER JOIN dvoo_towns t ON t.id = x.tid  
			WHERE x.day > 0 AND x.v < 10000'.
			(!is_null($sdates['start']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) > '.$sdates['start'] : '').
			(!is_null($sdates['end']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) < '.$sdates['end'] : '').
			' ORDER BY x.v DESC LIMIT 1');
	$r2 = $r2_sql[0];
	// highest defense (10)
	$r3_sql = $db->query(' 
			SELECT x.v, t.name, t.id, x.day 
			FROM dvoo_stat_zombies x 
			INNER JOIN dvoo_towns t ON t.id = x.tid  
			WHERE x.day > 0 AND x.v < 15000 AND x.v >= 10000'.
			(!is_null($sdates['start']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) > '.$sdates['start'] : '').
			(!is_null($sdates['end']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) < '.$sdates['end'] : '').
			' ORDER BY x.v DESC LIMIT 1');
	$r3 = $r3_sql[0];
	...
	$r4_sql = $db->query(' 
			...
			FROM dvoo_stat_zombies x 
			INNER JOIN dvoo_towns t ON t.id = x.tid  
			WHERE x.day > 0 AND x.v >= 15000'.
			(!is_null($sdates['start']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) > '.$sdates['start'] : '').
			(!is_null($sdates['end']) ? ' AND (t.stamp - ((t.day - 1) * 86400)) < '.$sdates['end'] : '').
			' ORDER BY x.v DESC LIMIT 1');
	$r4 = $r4_sql[0];
	
	print '<h3>'.t('SEASON').' '.$s.'</h3>';
	print '<table class="stat-towns-table">';
	print $r1 ? '<tr class="odd"><th>'.t('REC_ZOMBIE_COUNT').'</th><td>'.$r1['z'].' ('.$r1['name'].')</td><td>'.($r1['id'] >= 4560 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r1['id'].','.($r1['day']).');" />' : '').'</td></tr>' : '';
	print $r2 ? '<tr class="even"><th>'.t('REC_DEFENSE').'</th><td>'.$r2['v'].' ('.$r2['name'].')</td><td>'.($r2['id'] >= 4560 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r2['id'].','.($r2['day']+1).');" />' : '').'</td></tr>' : '';
	print $r3 ? '<tr class="odd"><th>'.t('REC_DEFENSE_10').'</th><td>'.$r3['v'].' ('.$r3['name'].')</td><td>'.($r3['id'] >= 4560 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r3['id'].','.($r3['day']+1).');" />' : '').'</td></tr>' : '';
	print $r4 ? '<tr class="even"><th>'.t('REC_DEFENSE_15').'</th><td>'.$r4['v'].' ('.$r4['name'].')</td><td>'.($r4['id'] >= 4560 ? '<img class="clickable" src="'.t('GAMESERVER_ITEM').'tagger.gif" onclick="spyontown('.$r4['id'].','.($r4['day']+1).');" />' : '').'</td></tr>' : '';
	print '</table>';
	
}

	// distances
	$q0 = $db->query('SELECT COUNT(*) AS c, (ROUND(SQRT((b.x - t.x)*(b.x - t.x) + (t.y - b.y)*(t.y - b.y)))) AS km FROM `dvoo_zones_buildings` b INNER JOIN dvoo_towns t ON t.id = b.tid WHERE b.type = 100 AND t.h > 15
	GROUP BY km ORDER BY c DESC');
	$q1 = $db->query('SELECT COUNT(*) AS c, (ROUND(SQRT((b.x - t.x)*(b.x - t.x) + (t.y - b.y)*(t.y - b.y)))) AS km FROM `dvoo_zones_buildings` b INNER JOIN dvoo_towns t ON t.id = b.tid WHERE b.type = 101 AND t.h > 15
	GROUP BY km ORDER BY c DESC');
	$q2 = $db->query('SELECT COUNT(*) AS c, (ROUND(SQRT((b.x - t.x)*(b.x - t.x) + (t.y - b.y)*(t.y - b.y)))) AS km FROM `dvoo_zones_buildings` b INNER JOIN dvoo_towns t ON t.id = b.tid WHERE b.type = 102 AND t.h > 15
	GROUP BY km ORDER BY c DESC');
	print '<h3>'.t('RUIN_DISTANCES').'</h3>';
	print '<table class="ruins-distances">';
	print '<tr><td>';
	print '<h4>'.t('BUNKER').'</h4><table class="stat-towns-table"><tr><th>'.t('DISTANCE').'</th><th>'.t('COUNT').'</th></tr>';
	$i = 0;
	foreach ($q0 AS $bd) {
		print '<tr class="'.($i % 2 == 0 ? 'even' : 'odd').'"><td>'.$bd['km'].t('KM').'</td><td>'.$bd['c'].'</td></tr>';
		$i++;
	}
	print '</table></td><td>';
	print '<h4>'.t('HOTEL').'</h4><table class="stat-towns-table"><tr><th>'.t('DISTANCE').'</th><th>'.t('COUNT').'</th></tr>';
	$i = 0;
	foreach ($q1 AS $bd) {
		print '<tr class="'.($i % 2 == 0 ? 'even' : 'odd').'"><td>'.$bd['km'].t('KM').'</td><td>'.$bd['c'].'</td></tr>';
		$i++;
	...
	print '</table></td><td>';
	print '<h4>'.t('HOSPITAL').'</h4><table class="stat-towns-table"><tr><th>'.t('DISTANCE').'</th><th>'.t('COUNT').'</th></tr>';
	$i = 0;
	foreach ($q2 AS $bd) {
		print '<tr class="'.($i % 2 == 0 ? 'even' : 'odd').'"><td>'.$bd['km'].t('KM').'</td><td>'.$bd['c'].'</td></tr>';
		$i++;
