...
foreach ( $build AS $bid ) {
	...
	$blb_copy .= ':hordes_fleche: '.$buildings[$bid]['name']."\n";
...
foreach ( $req_rsc AS $rri => $rrc ) {
	...
	$copyl .= ':hordes_*: '.$rsc_items[$rri]['name'].' x'.$rrc;
	if ( $rrc > $bank[$rri]['anzahl']) {
		...
		$copyl .= ' //('.t('ITEMS_MISSING', array('%d' => $diff, '%s' => array($diff, t('MISSES'), t('MISS')))).')//';
		$missing[$rri] = $diff;
	}
	else {
		$diff = $bank[$rri]['anzahl'] - $rrc;
		$outl .= ' <span class="plus">('.t('ITEMS_REMAINING', array('%d' => $diff, '%s' => array($diff, t('REMAINS'), t('REMAIN')))).')</span>';
		$copyl .= ' //('.t('ITEMS_REMAINING', array('%d' => $diff, '%s' => array($diff, t('REMAINS'), t('REMAIN')))).')//';
	}
	$outl .= '</li>';
	$copyl .= "\n\n";
...
$twinoidid = preg_replace('#([^_]+_([1-9][0-9]+)\.jpg)#i','$2',$data['user']['avatar']);

$copy .= '[rp=Oval Office Bauamt]//**+++ '.t('CONSTRUCTION_PERMIT').' +++**//'."\n".t('REQUESTED_BY', array('%s' => '@'.$data['user']['name'].':'.$twinoidid)).' - '.t('STATUS_QUO').': '.t('DAY').' '.$data['current_day'].', '.date('H:i',time()).t('TIME_APPENDIX').''."\n\n".t('CONSTRUCTIONS_PERMITTED', array('%d' => $bcnt, '%s' => array(1, t('HAS_BEEN'), t('HAVE_BEEN'))))."\n".$blb_copy."\n\n";
if ( $gen_vp > 0 ) {
	$copy .= t('VP_GAIN', array('%s' => '**'.$gen_vp.'**'));
	if ( $spent_deff > 0 ) { $copy .= ' //('.t('VP_USE', array('%d' => $spent_deff, '%s' => array(1, t('WILLS'), t('WILL')), '%t' => array(1, t('VP_ITEM'), t('VP_ITEMS')))).')//'; }
	$copy .= "\n\n";
}
$copy .= t('REQUIRED_AP_BUILD').': **'.$req_ap.'**'."\n\n".t('REQUIRED_RESOURCES')."\n\n";
$copy .= $copyl;
 ...
 */
	...
	if ( count($conv) > 0 ) {
		...
		foreach ( $conv AS $con ) {
			...
			$copy .= ':hordes_*: '.$con[2].'x '.$rsc_items[$con[0]]['name'].' :hordes_fleche: '.$rsc_items[$con[1]]['name'].($con[3] > 1 ? ' //('.t('CAUTION').': '.t('REQUIRED_STEPS', array('%d' => $con[3])).'//' : '')."\n";
			$wap += ($con[2] * $con[3]);
		}
		$out .= '</ul>';
		$out .= '<p>'.t('REQUIRED_AP_WORKSHOP').': <strong>'.($wap * $faktor).'</strong>';
		$copy .= t('REQUIRED_AP_WORKSHOP').': **'.($wap * $faktor).'**';
		if ( $manu || $saeg ) {
			$out .= ' <em>(';
			$copy .= ' //(';
		}
...
if ( $build_list != '' ) {
	print '<div id="grSection">An Hand der ausgewählten Gebäude können Einkaufslisten in der "Geheimen Ruine" generiert werden.<br/><button id="grButton" onclick=\'grList("'.trim(substr($build_list,1)).'");\'>Einkaufsliste erstellen</button><p id="grText"></p></div>';
}
