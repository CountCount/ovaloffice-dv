...
print '<p>'.$ac[0][0].' Gebäude des neuen Bausystems wurden im OO registriert, '.$bc[0][0].' davon wurden bereits verarbeitet.<br/>Wenn Ihr in Eurer Stadt einen Bauplan gefunden habt, dann klickt auf das rote Kreuz neben dem entsprechenden Bauplan in der Liste, um dieses Gebäude für den Baurechner freizugeben.</p>';
...
function render_building($b, &$pro, $l = 0, $p1 = 0, $p2 = 0, $p3 = 0, $c = 'rgba(0,0,0,0.1)') {
	...
	if ( is_array($pro) && !in_array($b['id'], $pro) ) {
		...
		if ( !in_array($b['id'],$tbuildings) ) {
			...
			if ( $data['town']['hard'] == 1 ) {
				print '</td><td>';
				print '<img class="clickable" src="css/img/invite-decline.png" title="'.t('MARK_AS_DESTROYED').'" onclick="destroyBuilding('.$b['id'].');">';
			}
		}
		print '</td><td>';
		if ( $l > 0 ) {
		...
		print '<img src="'.$data['system']['icon_url'].$b['img'].'.gif" />&nbsp;<abbr title=\''.$b['desc'].'\'>'.$b['name'].'</abbr>'.($b['temp'] ? '&nbsp;<img src="'.$data['system']['icon_url'].'small_warning.gif" title="'.t('TEMPORARY_BUILDING').'" />' : '').'</td><td class="tr">'.($b['vp'] > 0 ? $b['vp'].' <img src="http://www.dieverdammten.de/gfx/icons/small_def.gif" />' : '').'</td><td class="tr">';
...
$piwikTracker->setURL('http://dv.sindevel.com/oo/bau.ajax.php');
...
$piwikTracker->doTrackPageView('OO Construction Yard');