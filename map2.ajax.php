...
if ( $data = unserialize($session[0]['xml']) ) {
	...
	for ($y = 0; $y < $data['map']['height']; $y++) {
		for ($x = 0; $x < $data['map']['width']; $x++) {
			$zone = array(
				...
				'radius15' => get_radius($x,$y,$data['town']['x'],$data['town']['y'],11),
			);
			// check for DB entry
			...
			// 0. Edit zone Button
			$info .= ($zone['city'] == 0 ? '<div class="zib-full zib-box zone-edit-button-wrapper" style="background:#bbbbe9;"><button class="zone-edit-button" onclick="zoneEditLoadInfo('.$zone['x'].','.$zone['y'].','.$data['town']['id'].','.$data['current_day'].','.$data['user']['id'].');">ZONE BEARBEITEN</button></div>' : '');
			...
			$info .= (!is_null($zone['lastscout']) ? '<p title="'.$zone['lastscout_by'].', '.utf8_encode(strftime(t('DATETIME_FULL'), (int) ($zone['lastscout_on']))).t('TIME_APPENDIX').'"><strong>'.t('SCOUT_INFO').': '.$zone['lastscout'].' '.t('ZOMBIEX', array('%s' => array($zone['lastscout'], t('ZOMBIE'), t('ZOMBIES')))).'</strong></p>' : '' );
			
			
			if ( $zone['city'] == 0 && !is_null($zone['e']) && ($zone['e'] > $zone['z_proof'] || is_null($zone['z_proof'])) ) {
				$info .= '<p class="zombiekills"><img src="http://www.dieverdammten.de/gfx/forum/smiley/h_death.gif" /> Zombiekillrechner<span>';
				if ( $zone['e'] > 5 ) {
					$info .= 'Damit die Zone morgen zombiefrei ist, müssen heute '.ceil((2*$zone['e'] + 1)/3).' Zombies getötet werden.';
				}
				else {
					$info .= 'Damit die Zone morgen zombiefrei ist, müssen heute '.$zone['e'].' Zombies getötet werden.';
				}
				$info .= '</span></p>';
			}
			elseif ( $zone['city'] == 0 && !is_null($zone['z']) && $zone['z'] > 0 ) {
				$info .= '<p class="zombiekills"><img src="http://www.dieverdammten.de/gfx/forum/smiley/h_death.gif" /> Zombiekillrechner<span>';
				if ( $zone['z'] > 5 ) {
					$info .= 'Damit die Zone morgen zombiefrei ist, müssen heute '.ceil((2*$zone['z'] + 1)/3).' Zombies getötet werden.';
				}
				else {
					$info .= 'Damit die Zone morgen zombiefrei ist, müssen heute '.$zone['z'].' Zombies getötet werden.';
				}
				$info .= '</span></p>';
			}
			...
			if ($zone['city'] == 0) {
				...
				$today = $data['current_day'];
				$tday = date('z', time());
				$lday = date('z', $zone['lastvisit_on']);
				$tdiff = $lday <= $tday ? $tday - $lday : (date('L', $zone['lastvisit_on']) == 1 ? $tday + 365 - $lday : $tday + 364 - $lday);
				
				$info .= (!is_null($zone['lastvisit_on']) ? '<p>'.t('ZONE_LASTVISIT').': <strong>'.$zone['lastvisit_by'].'</strong> '.t('ON').' '.t('DAY').' '.($today - $tdiff).'<br/>('.utf8_encode(strftime(t('DATETIME_FULL'), (int) ($zone['lastvisit_on']))).t('TIME_APPENDIX').').</p>' : '');
				$info .= '</div>';
			}
			...
			// czu(\'load\','.$tid.','.$uid.','.$day.','.$x.','.$y.', '.$zone['e'].');
			...
			*/
			...
			if ($zone['city'] == 0) {
				...
				if ( is_array($zone['items']) ) {
					foreach ( $zone['items'] AS $item ) {				
						if ( in_array($item['id'], array(107,218,45,64,233,169,170,31,179,134,178,133,96,166)) ) {
							$vg += $item['count'];
						}
					}
				}
				if ( $vg > 0 ) {
					$out .= '<div class="zone-vg">'.$vg.'</div>';
				}
			}
			...
			foreach ( $zone['radius15'] AS $d => $c ) {
				if ( $c == 1 ) {
					$out .= '<div class="zone-radius15-border zone-border-'.$d.'"></div>';
				}
			}
	...
	print '<div id="zone-edit-box" class="hideme"><div id="zone-edit-tools"><div id="zone-edit-tools-content">';
	print 'Gegenstand hinzufügen<br/><input type="radio" id="add2zone" name="add2" checked="checked" value="1"> zur Zone (am Boden)';
	print '<br/><input type="radio" id="add2building" name="add2" value="2"> zum Gebäude (als Fundstück)';
	print '<div class="zone-edit-tools-cat clearfix">';
	...
	print '<a href="javascript:void(0);" id="map2_hover_vg" class="map_hover ms_on" onclick="return false;" >'.t('MAP_LAYER_VG').'</a>';
	...
	print '<a href="javascript:void(0);" id="map2_hover_radius15" class="map_hover ms_on" onclick="return false;" >'.t('MAP_LAYER_RADIUS15').'</a>';
	...
	$("#map2_hover_vg").click(function(el) {
		$("#map2_hover_vg").toggleClass("ms_on ms_off");
		$(".zone-vg").toggleClass("hideme");
	});
	...
	function getScrollTop() {
		...
		$(".zone-edit-tool-item").live("click", function(index) {
			if ($("input[name=add2]:checked").val() == 2 ) {
				buildingEditItem($(this).attr("id").substring(1),1);
			}
			else {
		...
		$(".current-building-itemlist-item").live("click", function(index) {
			buildingEditItem($(this).attr("id").substring(2),-1);
		});
		...
		function buildingEditItem(id,n) {
			var litem = $("#i"+id);
			var zitem = $("#bi"+id);
			var ditem = $("#ci"+id);
			if ( 0 in zitem ) {
				...
				var ilist = $("#current-building-items");
				var iform = $("#edit-zone-building");
				
				iform.append("<input class=\'zoneitem\' id=\'bi"+id+"\' type=\'hidden\' name=\'bi["+id+"]\' value=\'1\' />");
				ilist.append("<div id=\'ci"+id+"\' title=\'"+litem.attr("title")+"\' class=\'current-building-itemlist-item\' style=\'background:transparent url("+litem[0].src+") 0px 1px no-repeat;\'>x1</div>");
			}
		}
...
$piwikTracker->setURL('http://dv.sindevel.com/oo/map2.ajax.php');
...
$piwikTracker->doTrackPageView('OO Map v2');
