...
if ( $data = unserialize($session[0][0]) ) {
	...
	$luy = mktime(0,0,0,date('n',time()),date('j',time()),date('Y',time()));
	$yxml = $db->query(' SELECT stamp FROM dvoo_xml WHERE tid = '.$data['town']['id'].' AND stamp < '.$luy.' ORDER BY stamp DESC LIMIT 1 ');
	$ystamp = (int) $yxml[0][0];
	$ytime = date('H:i:s', $ystamp);
...
<div>'.t('GRAPH_TYPE').' <select id="gtype"><option value="1">'.t('GRAPH_BARS').'</option><option value="2">'.t('GRAPH_LINES').'</option></select></div><div id="bankgraph"></div></div>';
	...
	print '<div id="bank_watson">'.t('BANK_WATSON_MSG', array('%s' => $ytime)).'<br/>';
	print '<textarea id="watson-log"></textarea><button id="watson-start" onclick="watson();">FBI starten</button><p id="watson-wait" class="hideme">Bitte warten...</p>';
	print '<br style="clear:both;"/></div>';
	print '</div>';
	print '<script type="text/javascript">
	function bgraph(c) {
	...
	function watson() {
		$("#watson-button").toggleClass("hideme");
		$("#watson-wait").toggleClass("hideme");
		$.ajax({
			type: "POST",
			url: "bank.watson.ajax.php",
			data: "t='.$data['town']['id'].'&d='.$data['current_day'].'&l="+$("#watson-log").val(),
...
$piwikTracker->setURL('http://dv.sindevel.com/oo/bank.ajax.php');
...
$piwikTracker->doTrackPageView('OO Bank');