		...
		for ($x = 0; $x < $data['map']['width']; $x++) {
			...
			if (!is_null($cit)) {
				...
				foreach ($data['map'][$y][$x]['citizens'] AS $cid => $carray) {
					...
					$cl .= '<img class="zone-detail-img" src="http://www.dieverdammten.de/gfx/icons/item_'.$jobs[$carray['job']]['img'].'.gif" />' . $carray['name'];
				}
			}
			...
			$tempinfo .= (is_array($building) && $building['dig'] > 0 ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_digger.gif" /> <em>'.t('ZONE_DIG', array('%d' => $building['dig'])).'</em><br/>' : '');
			...
			$tempinfo .= '<br/>';
			$tempinfo .= ((isset($dried) && $dried == 1) ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_broken.gif" /> '.t('ZONE_DRIED').'<br/>' : '');
			...
			if ( isset($zoe) && $zoe > $zom ) {
				$tempinfo .= ( ($zoe > 0 && !is_null($zoe) && !($y - $data['town']['y'] == 0 && $x - $data['town']['x'] == 0)) ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_killz.gif" /> <span class=\'zi_zom\'>'.$zoe.' '.t('ZOMBIEX', array('%s' => array($zoe, t('ZOMBIE'), t('ZOMBIES')))).(isset($zom)? '</span><br/><span class=\'zi_zoe\'>(Letzte behördlich bestätigte Zählung: '.$zom.' '.t('ZOMBIEX', array('%s' => array($zom, t('ZOMBIE'), t('ZOMBIES')))).')' : '').'</span><br/>' : (isset($dan) ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_killz.gif" /> <span class=\'zi_zom\'>'.$zombies[$dan].' '.t('ZOMBIEX', array('%s' => array($zombies[$dan], t('ZOMBIE'), t('ZOMBIES')))).'</span><br/>' : '<span></span>'));
			}
			else {
				$tempinfo .= ( ($zom > 0 && !is_null($zom) && !($y - $data['town']['y'] == 0 && $x - $data['town']['x'] == 0)) ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_killz.gif" /> <span class=\'zi_zom\'>'.$zom.' '.t('ZOMBIEX', array('%s' => array($zom, t('ZOMBIE'), t('ZOMBIES')))).'</span><br/>' : (isset($dan) ? '<img class="zone-detail-img" src="http://data.dieverdammten.de/gfx/icons/r_killz.gif" /> <span class=\'zi_zom\'>'.$zombies[$dan].' '.t('ZOMBIEX', array('%s' => array($zombies[$dan], t('ZOMBIE'), t('ZOMBIES')))).'</span><br/>' : '<span></span>'));
			}
			$tempinfo .= (!is_null($cit) ? '<img class="zone-detail-img" src="http://www.dieverdammten.de/gfx/forum/smiley/h_human.gif" /> <span class=\'zi_hum\'>'.$cit.' '.t('CITIZEN').': '.$cl.'</span><br/>' : '');
			$tempinfo .= ($nyv?'<em>'.t('ZONE_NYV').'</em>':($nvt?'<em>'.t('ZONE_NVT').'</em>':''));
			$tempinfo .= (isset($time) ? '<br/>'.t('ZONE_LASTUPDATE').' '.utf8_encode(strftime("%e. %B %Y", (int) ($time))).' um '.date('H:i',$time).t('TIME_APPENDIX').'.' : '');
			$tempinfo .= (!is_null($data['map'][$y][$x]['visit_on']) ? '<br/>'.t('ZONE_LASTVISIT').': <strong>'.$data['map'][$y][$x]['visit_by'].'</strong> am '.utf8_encode(strftime("%e. %B %Y", (int) ($data['map'][$y][$x]['visit_on']))).' um '.date('H:i',$data['map'][$y][$x]['visit_on']).t('TIME_APPENDIX') : '');
			$tempinfo .= (!is_null($data['map'][$y][$x]['radar_on']) ? '<br/>'.t('ZONE_LASTRADAR').': <strong>'.$data['map'][$y][$x]['radar_by'].'</strong> am '.utf8_encode(strftime("%e. %B %Y", (int) ($data['map'][$y][$x]['radar_on']))).' um '.date('H:i',$data['map'][$y][$x]['radar_on']).t('TIME_APPENDIX') : '');
			...
			$tempinfo .= (!is_null($data['map'][$y][$x]['radar_r']) ? ($data['map'][$y][$x]['radar_r'] == 1 ? '<strong class="plus">'.t('ZONE_REGENERATED').'</strong>' : ($nyv?'<strong class="plus">'.t('ZONE_NEW_REGENERATED').'</strong>':'<strong class="minus">'.t('ZONE_NOT_REGENERATED').'</strong>')) : '' );
			...
			if ( !is_null($tag) && $tag > 0 ) {
				$ma['tags'] .= '<div class="map_tag" style="left:'.(2 + $x * 27).'px;top:'.(2 + $y * 27).'px;"><img src="http://data.dieverdammten.de/gfx/icons/tag_'.$tag.'.gif" /></div>';
			}			
...
if ( (date('z') >= 111 && date('z') <= 114) ) {
	...
	if ( $data['user']['id'] == 3137 || 1 == 1 ) {
		...
		/*print '<form id="add-item" onsubmit="addItem();return false;">
		<input type="hidden" name="formid" value="et-add-item" />
		<input type="hidden" name="t" value="'.$data['town']['id'].'" />
		<input type="hidden" name="u" value="'.$data['user']['id'].'" />
		<input type="hidden" name="d" value="'.$data['current_day'].'" />
		<input name="item-name" id="map-search2-item" type="text" /><input id="ms2-button" type="submit" value="'.t('MAP_SEARCH_FIND').'" />
	</form>';*/
	}
...
if ( (date('z') >= 111 && date('z') <= 114) ) {
	...
	/*$("#infoblock_zone")
		...
		/*$(function() {
			var availableItems = [ "'.implode('","', $aitems).'"	];
			$( "#map-search2-item" ).autocomplete({
